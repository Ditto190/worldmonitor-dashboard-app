...
每次每分钟拒绝都会原样回显你的 JSON-RPC `id` —— 字符串或数字，包括 `0` —— 因此它满足 MCP schema 的 `RequestId` 联合类型，你的客户端可以把待处理请求结算为限流错误，而不是让响应校验失败。唯一的例外是 `GET /mcp` 的 SSE 重放通道：它没有 JSON-RPC 请求体，也就没有 `id` 可回显，其拒绝仍保持 `"id": null`。

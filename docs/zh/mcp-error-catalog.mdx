...
| `-32029`  | 被限流 —— 每分钟节流或按套餐解析的每日配额上限        | **200**（每分钟）/ **429**（每日） | 遵循 `Retry-After`；对于 200/每分钟，退避约 1 秒                    |
...
每次每分钟拒绝都会原样回显你的 JSON-RPC `id` —— 字符串或数字，包括 `0` —— 因此它满足 MCP schema 的 `RequestId` 联合类型，你的客户端可以把待处理请求结算为限流错误，而不是让响应校验失败。有两种情况没有 `id` 可回显，其拒绝仍保持 `"id": null`：一是 `GET /mcp` 的 SSE 重放通道，它根本没有 JSON-RPC 请求体；二是**通知**（不带 `id` 发送的请求，例如 `notifications/initialized`）—— 被限流的通知会收到错误信封，而不是通常的无正文 `202`，并且按定义就没有 id 可关联。

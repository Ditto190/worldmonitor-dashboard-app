...
The scope of that guarantee is deliberate. The separate [docs MCP server](/agent-discovery) at `/api/docs-mcp` and the A2A endpoint at `/api/a2a` also answer `-32029`, but their limiters run *before* the request body is parsed, so they have no id in hand and still reply `"id": null`. Handle a null id defensively when you integrate against those two endpoints.

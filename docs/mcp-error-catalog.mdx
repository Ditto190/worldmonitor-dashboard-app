...
| `-32029`  | Rate-limited — per-minute throttle, plan-resolved daily cap, free-account allowance, OR anonymous free-tier ceiling | **200** (per-min) / **429** (daily, allowance, anon ceiling) | Honour `Retry-After`; for 200/per-min back off ~1s                    |
...
Every per-minute denial echoes your JSON-RPC `id` exactly — string or number, `0` included — so it satisfies the MCP schema's `RequestId` union and your client can settle the pending request as a rate-limit error instead of failing response validation. The one exception is the `GET /mcp` SSE replay channel, which carries no JSON-RPC body and therefore no `id` to echo; its denial keeps `"id": null`.
...
| **429** | JSON-RPC envelope         | `-32029`         | Plan-resolved daily cap, free-account allowance, anonymous free-tier ceiling, or pre-auth key-validation flood. `Retry-After` set (seconds-until-UTC-midnight for the daily/allowance cases). (Per-minute throttle returns `-32029` inside HTTP 200 — see `-32029` above.) |

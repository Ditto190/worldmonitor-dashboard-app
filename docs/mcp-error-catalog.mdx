...
Every per-minute denial echoes your JSON-RPC `id` exactly — string or number, `0` included — so it satisfies the MCP schema's `RequestId` union and your client can settle the pending request as a rate-limit error instead of failing response validation. Two cases have no `id` to echo and keep `"id": null`: the `GET /mcp` SSE replay channel, which carries no JSON-RPC body at all, and a **notification** (a request sent with no `id`, such as `notifications/initialized`) — a notification that is rate-limited gets the error envelope rather than its usual bodyless `202`, and by definition has no id to correlate.
